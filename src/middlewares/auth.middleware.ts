...
export const verifyToken = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (accessToken) {
            ...
            if (!payload) return res.status(404).send({ error: "Invalid or expired token", code: 404 })

            req["user"] = payload

            let existUser = await UserModel.findOne({ userName: req["user"].userName });
            if (!existUser) return res.status(404).send({ code: 404, error: "User doesn't exist or has been removed" })
        ...
        return res.status(500).send({ code: 500, message: "Error verifying token", error })
    }
}